...
`);
    ...
    class L {
      ...
      async OpenAI(e = [], t = this.Source, a = this.Target, n = this.API) {
        e = Array.isArray(e) ? e : [e];
        let s = (n?.base_url ?? n?.baseUrl ?? n?.BaseURL ?? n?.Endpoint ?? n?.endpoint ?? "").toString().replace(/\/+$/, "");
        if (!s) {
          throw Error("OpenAI 配置缺失: base_url");
        }
        let r = (n?.api_key ?? n?.apiKey ?? n?.APIKey ?? n?.Key ?? n?.Auth ?? "").toString();
        let i = (n?.model ?? n?.Model ?? "gpt-4o-mini").toString();
        ...
        if (r) {
          u.headers.Authorization = `Bearer ${r}`;
        }
        let g = t && t !== "AUTO" ? `Source language: ${t}.` : "Detect the source language.";
        let c = `You are a translation engine for subtitles.\n${g}\nTarget language: ${a}.\nRules:\n- Translate each item of the JSON array.\n- Return ONLY a valid JSON array of strings.\n- Keep the array length exactly the same.\n- Do not add numbering, explanations, or code fences.\n- Preserve line breaks inside each item if present.`;
        u.body = JSON.stringify({
          ...
          temperature: n?.temperature ?? n?.Temperature ?? 0
        });
        return await l(u).then(t => {
          let a = JSON.parse(t.body ?? "{}");
          let n = a?.choices?.[0]?.message?.content ?? a?.choices?.[0]?.text ?? a?.output_text ?? a?.text ?? "";
          if (!n) {
            throw Error("OpenAI 响应解析失败: 缺少 content");
          }
          n = n.trim().replace(/^```(?:json)?\\s*/i, "").replace(/```\\s*$/i, "").trim();
          let s;
          try {
            s = JSON.parse(n);
          } catch (e) {
          ...
          if (s.length !== e.length) {
            ...
            while (s.length < e.length) s.push("​");
          }
          return s.map(e => typeof e == "string" ? e : JSON.stringify(e));
        }).catch(e => Promise.reject(e));
      }
    }
